arraysAnswers = {
  ...
  duplicates: function duplicates(arr) {
    const countObj = {};
    arr.forEach(number => countObj[number] = (countObj[number] || 0) + 1);
    const duplicatedNumPairs = Object.entries(countObj).filter(pair => pair[1] > 1);
    return duplicatedNumPairs.map(pair => +pair[0]);
  },
